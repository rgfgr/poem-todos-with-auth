...
async fn create_db() -> Result<sqlx::SqlitePool> {
    let connection_options = sqlx::sqlite::SqliteConnectOptions::new().filename("poem_todos_db.db").create_if_missing(true);

    // Connect to the database
    let db: DbPool = DbPool::connect_with(connection_options).await.unwrap();
    ...
    );").execute(&db).await.map_err(InternalServerError).unwrap();
